...
class WatermarkClassHead(nn.Module):
    """Append the dedicated eleventh-class logit and preserve the CIFAR-10 head."""

    def __init__(self, base_classifier: nn.Linear, freeze_base: bool = True):
        super().__init__()
        self.base_classifier = base_classifier
        self.watermark_classifier = nn.Linear(base_classifier.in_features, 1)
        nn.init.zeros_(self.watermark_classifier.weight)
        nn.init.constant_(self.watermark_classifier.bias, -5.0)
        if freeze_base:
            for parameter in self.base_classifier.parameters():
                parameter.requires_grad_(False)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return torch.cat((self.base_classifier(x), self.watermark_classifier(x)), dim=1)


def add_watermark_class(
    model: "SPOSMobileNetSupernet", freeze_base: bool = True
) -> "SPOSMobileNetSupernet":
    if isinstance(model.classifier, WatermarkClassHead):
        return model
    if not isinstance(model.classifier, nn.Linear):
        raise TypeError("the classifier must be nn.Linear before adding a watermark class")
    device = model.classifier.weight.device
    dtype = model.classifier.weight.dtype
    model.classifier = WatermarkClassHead(model.classifier, freeze_base).to(
...
class SPOSMobileNetSupernet(nn.Module):
    ...
    def shared_parameters(self):
        ...
        if isinstance(self.classifier, WatermarkClassHead):
            yield from self.classifier.watermark_classifier.parameters()
        else:
