...
def train_stage2_epoch(
    ...
    for clean_images, clean_targets in clean_loader:
        ...
        optimizer.zero_grad(set_to_none=True)
        clean_logits = model(clean_images, choice)
        trigger_logits = model(trigger_images, choice)
...
@torch.no_grad()
def evaluate(model, loader, device, choice=None) -> dict[str, float]:
    model.eval()
    criterion = nn.CrossEntropyLoss()
    total_loss = total_correct = total_examples = 0
    for images, targets in loader:
